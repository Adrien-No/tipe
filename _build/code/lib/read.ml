...
let get_graph path : graph =
  ...
  Array.iteri (fun i x -> Hashtbl.add id_sommets x.id (i) ) nodes;

  let ladj = Array.make (Array.length nodes) [] in
  let inserer_arete (edge:route_seule) : unit =
    (* insère la double arête edge dans ladj *)
    let x = nodes.(Hashtbl.find id_sommets edge.source).id
    and y = nodes.(Hashtbl.find id_sommets edge.target).id in
    begin
      if edge.car_forward > 0 then
        ladj.(x) <- {cible=y;length=edge.length;voies=edge.car_forward}::ladj.(x);
      if edge.car_backward > 0 then
        ladj.(y) <- {cible=x;length=edge.length;voies=edge.car_backward}::ladj.(y);
    end
  in
